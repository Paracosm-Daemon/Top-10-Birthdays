...
class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		[
			'Add Camera Zoom',
			"Used on MILF on that one \"hard\" part\nValue 1: Camera zoom add (Default: .015)\nValue 2: UI zoom add (Default: .03)\nLeave the values blank if you want to use Default."
		],
		[
			'Change Default Zoom',
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			if (#if sysFileSystem.exists(file) #else OpenFlAssets.exists(file) #end)
			{
				clearEvents();
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (curSelectedNote != null && curSelectedNote[1] > -1)
			{
				var delta:Int = CoolUtil.boolToInt(FlxG.keys.justPressed.E) - CoolUtil.boolToInt(FlxG.keys.justPressed.Q);
				if (delta != 0)
			...
			if (FlxG.keys.justPressed.BACKSPACE || FlxG.keys.justPressed.ESCAPE)
			{
				...
				// if (onMasterEditor) {
				MusicBeatState.switchState(new #if debug editors.MasterEditorMenu #else MainMenuState #end());
	...
	*/
	...
	function loadHealthIconFromCharacter(char:String) {
		...
		if (!OpenFlAssets.exists(path)) path = Paths.getPreloadPath('characters/' + Character.DEFAULT_CHARACTER + '.json'); //If a character couldn't be found, change him to BF just to prevent a crash
